<template>
  <v-app>
    ...
    <v-navigation-drawer v-model="isMenuToggled" app clipped>
      <v-list-item two-line v-if="viewModel.token">
        <v-list-item-avatar tile>
          <v-img :src="`data:image/jpeg;base64,${viewModel.user.avatar}`"/>
        </v-list-item-avatar>

        <v-list-item-content>
          <v-list-item-title class="title">{{ viewModel.user.name }}</v-list-item-title>
          <v-list-item-subtitle>{{ viewModel.user.email }}</v-list-item-subtitle>
        </v-list-item-content>
      </v-list-item>
